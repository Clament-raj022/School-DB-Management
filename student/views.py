...
def adlogin(request ):
    ...
    if a.is_valid():
        ...
        if(mem=="Teacher"):
            ...
            a=stud_registration_model.objects.all().filter(your_register_no_is=uname,date_of_birth=passw).values("id","name","mail")
            if not a:
                messages.warning(request,"Invalid UserName or Password")
                return login(request)
            else:
...
def stud_reg(request):
    ...
    if a.is_valid():
        ...
        a.save()
        messages.success(request,'Student details Registered Successfully')
...
def studentdetails(request):
    a=stud_registration_model.objects.all().filter(status=1)
    print(a)
...
def edit_stu(request):
    a=request.GET["q"]
    b=stud_registration_model.objects.all().filter(id=a)
    print(b)
    return render(request,'student/stu_edit.html',{'data':b})
def edi_stu(request):
    a=request.POST['id']
    b=stud_registration_model.objects.get(id=a)
    print(b)
    c=stud_registration_form(request.POST,request.FILES or None,instance=b)
    if(c.is_valid()):
        ...
        messages.error(request,'Student details updation Failed')
    return studentdetails(request)

  
def del_mark(request):
    a=request.GET["q"]
    print("Id is ",a)
    student_marks_register_model.objects.all().filter(id=a).delete()
    return render (request,'student/stu_view_marks.html')
