...
def teacherdetails(request):
    a=registration_model.objects.all().filter(status=1)
    print(a)
...
def edi_cust(request):
    ...
    if(c.is_valid()):
        ...
        messages.error(request,'Teacher details updation Failed')
    return teacherdetails(request)
...
def mark_register(request):
    a=request.GET["v"]
    c=student_marks_register_model.objects.all().filter(stu_id=a)
    if not c:
        ...
        messages.error(request,'Marks already Exists')
        return stu_view_tea(request)
def mark_reg(request):
